...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tarvitset aseen jossa on ammuksia!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("panttivanki:sync",targetSrc)
			...
			drawNativeNotification("~r~Ei ket‰‰n l‰hell‰ panttivangiksi!")
		end
	end
end 

RegisterNetEvent("panttivanki:syncTarget")
AddEventHandler("panttivanki:syncTarget", function(target)
